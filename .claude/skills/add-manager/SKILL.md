...
2. Walk the **file checklist** below and check **every** file for the manager's presence. The most commonly missed files are: `docs/meta_package_manager.managers.md`, `labels.py` (group and synonyms), `test_pool.py` (manager count), and `changelog.md` — plus a `docs/docs_update.py` run to regenerate the pool-derived blocks (readme, keywords, labeller rules, manager page stubs).
