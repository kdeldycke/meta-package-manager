...
| `installed` output carries a version for each package.                                                   | The listing is name-only (`apt-cyg list`, `swupd bundle-list`, `deb-get list --installed`): the DSL requires an `installed_version` capture, while a class may yield packages without one. Resolving versions through a follow-up call per package (`pacstall --cache-info`) is equally out of reach: one operation is one command. |
