...
Then run `docs/docs_update.py` to regenerate the readme Sankey diagram and the operations matrix. The benchmark page's manager-support table needs no regeneration: it renders live at Sphinx build time. A bundled config manager needs **no** `pool.py` import, `labels.py` group, label rules, or `docs/meta_package_manager.managers.md` automodule: those are class-only.
