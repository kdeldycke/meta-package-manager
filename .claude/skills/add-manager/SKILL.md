...
| `docs/stars_update.py`                    | Add the manager's upstream repository to `UPSTREAM_REPOS`, or its reason for having none to `NO_UPSTREAM`. `test_manager_upstreams_cover_the_pool` requires every wrapped manager to sit in exactly one of the two, since the pair feeds the popularity and activity columns of the manager index.                                          |
| `docs/cooldown.md`                        | Two hand-maintained tables, each needing a row. The *Supported managers* table takes the manager's cooldown status (`❌ None` unless the tool has a release-age gate of its own), and `manager_cooldown()` reuses that row on the manager's own page, so omitting it silently drops the section. The *Retraction paths by registry* table takes the manager's id in whichever registry row its packages come from; `test_retraction_table_well_formed` fails until it is there, since the mapping must partition the pool. |
