...
| `docs/cooldown.md`                        | Two hand-maintained tables, each needing a row. The *Supported managers* table takes the manager's cooldown status, a three-way call settled during vetting (see *Gating the manager under `--cooldown`*): ✅ native knob, ✅ `mpm` per-package probe, or ❌/➖. `manager_cooldown()` reuses that row on the manager's own page, so omitting it silently drops the section, and **no test enforces this row**: `pikaur`, `trizen` and `dkp-pacman` all shipped without one. The *Retraction paths by registry* table takes the manager's id in whichever registry row its packages come from, and its *Publish date* cell is worth filling while the registry research is fresh, a server-set date there being exactly what qualifies the probe; `test_retraction_table_well_formed` fails until the id is there, since the mapping must partition the pool. |
