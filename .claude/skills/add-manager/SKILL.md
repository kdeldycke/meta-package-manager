...
| `installed` output carries a version for each package.                                                   | The listing is name-only (`apt-cyg list`, `swupd bundle-list`): the DSL requires an `installed_version` capture, while a class may yield packages without one.                  |
| Every mutating operation is one argument list with `{package_id}` or `{query}` substituted in.          | An operation needs *conditional* `sudo` (static `sudo = true` is fine), delegation, or output post-processing.                                                                  |
...
| `docs/augmentations.md`                                    | The class synthesizes search filtering (`@search_capabilities(extended_support=False, exact_support=False)`) or a free `upgrade --all`.   | Add the manager's row with the matching ✅ columns.                                                  |
