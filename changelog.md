...
- [gem] Note on the manager's page that on recent macOS the system Ruby's default gem scope lives on the sealed read-only system volume, so its bundled gems surface as outdated yet cannot be upgraded in place, even with `sudo`. The note points to a user-controlled Ruby (Homebrew, `rbenv` or `rvm`) as the fix.
