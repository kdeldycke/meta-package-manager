...
- [asdf] Add [asdf](https://asdf-vm.com) package manager with `installed`, `outdated`, `search`, `install`, `upgrade`, `upgrade_all`, `remove`, and `sync` support. Targets Linux and macOS; requires `asdf` `>=0.16.0`, the Go rewrite that replaced the hyphenated subcommands (`asdf list-all`, `asdf plugin-add`) with their space-separated equivalents (`asdf list all`, `asdf plugin add`) the wrapper depends on. Each `(plugin, installed_version)` pair is reported as a distinct package. `outdated` only surfaces tools whose currently-active version (marked with `*` in `asdf list`) differs from the latest stable release; tools installed but never activated through a `.tool-versions` file are not reported as outdated. `install` does not auto-add plugins: `asdf plugin add <plugin>` must be run beforehand. `remove` runs `asdf plugin remove <plugin>`, which deletes the plugin metadata and every installed version of the tool. `search` enumerates the short-name plugin catalogue via `asdf plugin list all` and lets the framework refilter locally.
- [mise] Add [mise](https://mise.jdx.dev) package manager with `installed`, `outdated`, `search`, `install`, `upgrade`, `upgrade_all`, `remove`, `sync`, and `cleanup` support. Cross-platform on Linux, macOS, and Windows; requires `mise` `>=2025.5.10`, the first release that ships `mise search` and emits valid `mise outdated --json` when no tool is outdated. The wrapper targets globally-installed tool versions and never writes to `mise.toml`: `install` uses `mise install <tool>@<version>` (not `mise use`), `sync` runs `mise plugins update`, and `cleanup` runs `mise cache clear`. Backend-prefixed tool IDs (`pipx:ruff`, `cargo:ubi-cli`) round-trip as-is. `mise outdated --json` only reports tools tracked in a `mise.toml`, so bare-installed tools that were never pinned with `mise use` are absent from `mpm outdated --mise`.
- [mpm] Surface the `--man` and `--accessible` global options now provided by `click-extra`. `mpm --man` prints the command's man page to stdout in roff (pipe through `man --local-file -` to render), and `--accessible` (also enabled by `ACCESSIBLE=1`) strips ANSI colors and renders tables without Unicode box-drawing characters for screen-reader compatibility. Each tagged release now also attaches an `mpm-manpages.tar.gz` asset with static `.1` files for the full command tree, produced by the `manpages` job in `repomatic`'s `_release-engine.yaml` and opted into via `[tool.repomatic.manpages] script = "meta_package_manager.cli:mpm"` in `pyproject.toml`. The Sphinx HTML build emits the same pages alongside the docs site via the `click_extra_manpages` config flag (one `.1` per (sub)command plus a browser-viewable `.html` sibling rendered by `mandoc`/`groff` when available), surfaces a live index at the bottom of [CLI parameters](https://kdeldycke.github.io/meta-package-manager/cli-parameters.html#man-pages) via the `{click-extra-manpages}` directive, and wires Sphinx's standard `:manpage:` role to the HTML siblings through `manpages_url = "man/{page}.{section}.html"`. Bumps the `click-extra` floor to `>=7.19` to declare the dependency on the new Sphinx surface.
- [mpm] Add a `--cooldown DURATION` option and `[mpm] cooldown` config key that refuse to install or upgrade any package version published more recently than the given release age, as a mitigation against supply-chain attacks. Accepts a friendly duration (`7 days`, `1 week`, `12h`), an ISO 8601 duration (`P7D`, `PT12H`), or an RFC 3339 absolute timestamp (`2024-05-01T00:00:00Z`); calendar units like months and years are rejected because their length is ambiguous. Enforced through each manager's native release-age mechanism: `uv` and `uvx` (`UV_EXCLUDE_NEWER`), `npm` (`min-release-age`), and `pip` and `pipx` (`PIP_UPLOADED_PRIOR_TO`). Managers without native support are skipped during install and upgrade by default (fail-closed); pass `--allow-no-cooldown` (or set `[mpm] allow_no_cooldown`) to run them without the safeguard. See the new [Release-age cooldown](https://kdeldycke.github.io/meta-package-manager/cooldown.html) docs page for the per-manager support table.
- [brew] Pass `--yes` to `brew upgrade` to skip the new interactive confirmation prompt that ask mode introduced as the default behaviour in [`6.0.0`](https://brew.sh/2026/06/11/homebrew-6.0.0/). Bumps the minimum required `brew` version from `2.7.0` to `6.0.0`, the first release to ship the `--yes` opt-out flag. Older `brew` releases would either error on the unknown flag or, on `5.1.14`/`5.1.15`, hang on a `Do you want to proceed with the upgrade? [y/n]` prompt when run non-interactively from the SwiftBar plugin or any other automation harness.
- [brew] Trust tap-qualified formulae and casks before install. Homebrew `6.0.0` rejects code from third-party taps until each tap (or each formula or cask) has been explicitly trusted, so `brew install user/tap/name` would otherwise abort with a `tap trust is required` warning when the snapshot pins a third-party package like `gromgit/fuse/ntfs-3g-mac` or `smudge/smudge/nightlight`. `mpm` now routes every tap-qualified install through a new `trust_tap()` helper that runs `brew tap user/tap` (idempotent) followed by `brew trust user/tap/name --formula` or `--cask` before the install proceeds. Core formulae and casks (`wget`, `ffmpeg`, `firefox`, ...) live on the trusted `homebrew/core` and `homebrew/cask` taps and skip the trust step.
- [npm] Bump the minimum required `npm` version from `4.0.0` to `11.10.0`, the first release to ship the `min-release-age` config setting `mpm` now uses for the supply-chain cooldown. Older `npm` releases would silently ignore the gate.
- [pip] Bump the minimum required `pip` version from `10.0.0` to `26.1.0`, the first release to ship the `--uploaded-prior-to` resolver flag `mpm` now uses for the supply-chain cooldown. Older `pip` releases would silently ignore the gate.
...
- [mpm] Rename the global `--stats / --no-stats` flag to `--summary / --no-summary`, and move the rendering surface from `meta_package_manager/output.py` into a dedicated `meta_package_manager/summary.py` module. The flag now gates a count line plus optional subcommand-specific follow-up notes (SBOM enrichment / merge counts, dependency-graph edges), and "summary" describes that artifact more honestly than "stats" did. Breaking change for scripts that pass the old flag. The underlying SBOM data accessor `SBOM.stats()` keeps its name on purpose: it returns the raw count dict, distinct from the rendered summary the flag controls.
- [mpm] Make `mpm sbom` maximalist by default. The renderer now collects rich per-package metadata (license, supplier, originator, homepage, source URL, checksums, declared dependency graph) from each manager and embeds it in both SPDX and CycloneDX exports. Adds a `--bundled / --minimal` flag (default `--bundled`) so the previous bare inventory shape is still available for fast snapshots and scripted use. The download-location placeholder (`https://www.example.com`) is replaced with `NOASSERTION` when no real URL is known, and unparseable or undeclared license strings (including `LicenseRef-*`) collapse to `NOASSERTION` to keep document validation clean.
- [brew] Implement the maximalist SBOM extractor: a single `brew info --json=v2 --installed` shell-out populates each formula's license, homepage, supplier, source URL, bottle SHA-256, runtime/build/test/optional/recommended dependency edges, and `tap` / `keg_only` extras; the cask code path covers `--cask`. When `HOMEBREW_SBOM=1` was set at install time, `mpm sbom` discovers each `<prefix>/Cellar/<formula>/<version>/sbom.spdx.json` file, splices its transitive dependency closure into the aggregate document under a `SPDXRef-brew-<formula>-<dep>` namespace, and records the source file (with SHA-1) in `externalDocumentRefs` so the merge is auditable. CycloneDX exports attach the same file via an `externalReferences[type=bom]` entry on each component.
- [pip] Implement the SBOM extractor through `importlib.metadata` (no shell-outs): each installed distribution's `METADATA` file populates license, homepage, VCS / issue-tracker URL, author / maintainer originator, summary, and `Requires-Dist` dependency edges; PEP 508 dependency specs are split so `target_id` carries the bare name and `version_constraint` carries the rest.
- [mpm] Bump the CycloneDX SBOM output to schema `1.7` (was `1.5` for JSON and `1.6` for XML), the latest version published by the [CycloneDX specification](https://cyclonedx.org/docs/1.7). JSON exports now declare `"specVersion": "1.7"` and XML exports use the `http://cyclonedx.org/schema/bom/1.7` namespace. Raises the `mpm[sbom]` floor on `cyclonedx-python-lib` from `>=11.2` to `>=11.4`, the first release to expose the `JsonV1Dot7` outputter and `SchemaVersion.V1_7` enum.
...
- [mpm] Move `cyclonedx-python-lib` and `spdx-tools` out of the runtime install into a new `[sbom]` extra: `pip install meta-package-manager` no longer pulls CycloneDX or SPDX, and `mpm sbom` requires `pip install meta-package-manager[sbom]`. `packageurl-python` stays in the runtime install since it is also used by `meta_package_manager.package` and `meta_package_manager.specifier` for cross-manager package identification. Breaking change for anyone that relied on `mpm sbom` from a default install.
- [mpm] Reduce default-verbosity noise on operational subcommands. Underlying CLIs' captured stderr (gem extension warnings, mas Spotlight chatter, pip `No module named pip` errors, yarn `env: node: No such file`) now logs at DEBUG instead of WARNING/ERROR: failed runs still raise `CLIError` or accumulate in `manager.cli_errors`, so the data survives without flooding the table the user came to see. The per-manager `Skip <id> manager: <reason>` and `<id> does not implement Operations.<op>` messages also drop to DEBUG for implicit selection (`mpm outdated` with no `-m` flag) but stay at INFO/WARNING when the user explicitly targeted the manager (`mpm --cargo outdated`), so an explicit ask that cannot be fulfilled is never silent. A one-line `N managers reported errors during this run (<ids>); re-run with --verbosity DEBUG for details.` summary fires at the end of any subcommand whose underlying CLIs accumulated errors, preserving the failure signal that captured stderr previously provided.
...
- [mpm] Add `mpm config-template [manager-ids...]` subcommand that prints every overridable attribute of each requested manager as a ready-to-paste `[mpm.managers.<id>]` TOML block. With no positional argument, every maintained manager is dumped. Doubles as the canonical reference for what each manager exposes: users can pipe the output into a config file and prune the rows that don't apply.
- [mpm] Add `mpm dump` snapshot subcommand that exports the installed-package inventory as either a TOML manifest (`--toml`, default) or a Brewfile (`--brewfile`) compatible with `brew bundle install`. The Brewfile path includes the subset of managers natively supported by [Homebrew Bundle](https://brew.sh/2026/06/11/homebrew-6.0.0/) (`brew`, `cask`, `mas`, `vscode`, `npm`, `cargo`, `uv`, `winget`, `flatpak`); other managers are tallied in the file header but excluded from the output. The header carries a warning against running `brew bundle cleanup` on the file, since cleanup would treat the excluded managers' packages as "should be uninstalled". `mpm backup`, `mpm lock`, `mpm freeze`, and `mpm snapshot` are now aliases of `mpm dump`; all of `backup`'s flags (`--merge`, `--update-version`, `--overwrite`) are carried over to `mpm dump --toml`, and a `--header`/`--no-header` switch controls the metadata comment block at the top of the output.
...
- [mpm] Remove automated publishing of the Chocolatey package: the `🍫 Chocolatey` job is gone from `release.yaml`, and the `choco` job that installed from the community repository is gone from `tests-install.yaml`. [Submission `6.4.2`](https://community.chocolatey.org/packages/meta-package-manager/6.4.2) was rejected because the Windows x64 binary is flagged by more than 10 antivirus engines on VirusTotal, [the cutoff for community-repository approval](https://github.com/chocolatey/home/issues/395#issuecomment-4378555157). The in-tree `packaging/choco/meta-package-manager/` nuspec and the `choco-source` build-from-source job in `tests-install.yaml` are kept; install instructions in `docs/install.md` now point only at the local build path.
...
- [xbps] Add Void Linux build-from-source installation instructions to `docs/install.md`. The package is pending upstream review at https://github.com/void-linux/void-packages/pull/60532; until that merges, users can build from the `mpm` branch of the fork.
