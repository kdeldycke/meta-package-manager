...
- [mpm] Read-only operations (`installed`, `outdated`, `search`) now query managers in parallel, as does the manager-availability probe, via a new `--jobs`/`-j` option (default: CPU count minus one); the independent maintenance operations (`sync`, `cleanup`, `upgrade --all`) parallelize too, while the ordering-bound state changers (`install`, `remove`, `upgrade <packages>`, `restore`) stay sequential. Refs {issue}`529`.
