...
- [cpan] Document on the manager's page why the macOS system Perl reports its core modules (`warnings`, `feature`, `POSIX`, `B`, and the like) as permanently outdated and why `upgrade --all` never clears them: these modules ship only inside Perl, on the sealed read-only system volume, with no standalone CPAN distribution, so `cpan` can never upgrade them in place. The page advises disabling the manager or switching to a user-controlled Perl. Addresses [#1983](https://github.com/kdeldycke/meta-package-manager/issues/1983).
