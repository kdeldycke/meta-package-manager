...
- [mpm] `remove`, `upgrade`, `sync`, `cleanup` and `restore` gained the same interactive `✓`/`✗` trail and finisher as `install`: per-package for `remove`, `upgrade <packages>` and `restore` (`✓ foo removed from brew`), per-manager for `sync`, `cleanup` and `upgrade --all` (`✓ brew` / `✓ Synced N/M managers`). Per-attempt detail drops to `DEBUG`; `restore` stays best-effort (a failed package marks its line `✗` but does not fail the run).
- [mpm] `upgrade <packages>` now exits non-zero when a manager that has a requested package installed fails to upgrade it (previously always `0`), matching `install` and `remove`.
