...
- [mpm] Filter the package listing by a query: `installed` and `outdated` accept an optional `QUERY` argument, while `dump`/`backup` and `sbom` accept a `--query` option (their positional argument is already the output path). The match is fuzzy by default (case-insensitive, tokenized), with `--exact`/`--fuzzy` for control, reusing `search`'s semantics. In the `installed` and `outdated` tables, matched substrings are highlighted as in `search` results.
...
- [mpm] Operations now run managers in parallel via a new `--jobs`/`-j` option (default: CPU count minus one): the read-only queries (`installed`, `outdated`, `search`) and the manager-availability probe; the maintenance commands (`sync`, `cleanup`, `upgrade --all`); the inventory exporters (`dump`/`backup`, `sbom`); and the state changers (`install`, `remove`, `upgrade <packages>`, `restore`), which fan out across managers while running each manager's own packages one at a time. Only `install` of a package left untied to a manager stays sequential (it needs a priority search across managers) and logs an explicit `--jobs` as having no effect. Closes {issue}`529`.
...
- [mpm] Lower the default verbosity from `INFO` to `WARNING` and harmonize log levels into a three-tier model: the default shows only the `✓`/`✗` trail, finishers, and genuine warnings/criticals (the trail prints via `echo`, so it is unaffected); all per-operation narration (selection summary, `Installation priority`, per-manager announcements, discovery, capability skips like `X does not implement Y`, and ignored-option notices) sits at `INFO`; raw technical detail (CLI commands, result refiltering, internal data) sits at `DEBUG`. Operation names also render bare (`outdated`) instead of `Operations.outdated` in messages.
- [mpm] At `DEBUG` verbosity (`--verbosity DEBUG` or repeated `-v`), managers run sequentially and the progress spinner is hidden so interleaved per-manager logs stay readable; serialized output (`--table-format json`, etc.) also keeps its log narration on stderr.
- [mpm] `install` now prints a per-attempt `✓`/`✗` trail (one line per package and manager tried, like `✘ foo not found in brew` then `✓ foo installed with cask`) and a persistent `Installed N/M packages` finisher on an interactive terminal, replacing the per-package `Install … with …` info logs.
- [mpm] `remove`, `upgrade`, `sync`, `cleanup` and `restore` gained the same interactive `✓`/`✗` trail and finisher as `install`: per-package for `remove`, `upgrade <packages>` and `restore` (`✓ foo removed from brew`), per-manager for `sync`, `cleanup` and `upgrade --all` (`✓ brew` / `✓ Synced N/M managers`).
- [mpm] `upgrade <packages>` and `restore` now exit non-zero when a targeted package fails to upgrade/install (previously always `0`), matching `install` and `remove`.
- [mpm] Fix `install`/`remove`/`upgrade` silently dropping all-but-one target when the same package is requested with several explicit managers: `mpm install pkg:uv/rich pkg:brew/rich` now acts on both. A single alias pURL that maps to several managers (`pkg:rpm/ping` → `dnf`/`yum`/`zypper`) still reduces to the top-priority one.
...
- [mpm] Resolve `pkg:apk` purls to the `apk` manager and `pkg:npm` purls to `pnpm` (alongside `npm`, `yarn`, and `yarn-berry`); both were missing from the purl-to-manager map.
