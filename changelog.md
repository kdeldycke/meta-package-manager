...
- [mpm] Add a hermetic test that replays each manager's documented `installed` and `--version` sample output back through its own parser, so a regex drifting from the `shell-session` example beside it now fails CI. First step toward [#1023](https://github.com/kdeldycke/meta-package-manager/issues/1023).
