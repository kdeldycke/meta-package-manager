...
## [`6.5.0.dev0` (unreleased)](https://github.com/kdeldycke/meta-package-manager/compare/v6.4.3...main)

> [!WARNING]
> This version is **not released yet** and is under active development.

- [mpm] Use `CREATE_NEW_PROCESS_GROUP` + `KeyboardInterrupt` absorb + `SW_HIDE` when spawning subprocesses on Windows, replacing `CREATE_NEW_CONSOLE`. The history: `DETACHED_PROCESS` removed the child from any console which fixed console-signal leaks but broke Chocolatey shims (like `cargo.EXE`) that redirect output away from pipes when they detect no console, making version detection fail with "could not parse version". `CREATE_NEW_CONSOLE` kept the child on its own console so shims worked, but CI still saw test runs exit with code 1 after all tests passed, likely because winget and similar installers spawn finalizer processes whose `GenerateConsoleCtrlEvent` calls reach the parent despite the console isolation. `CREATE_NEW_PROCESS_GROUP` disables `CTRL_C` for the child's group while still inheriting the parent's console (so shims route output to pipes), and the new `except KeyboardInterrupt` block in `run()` absorbs any residual `CTRL_C_EVENT` that does reach Python without aborting the test run.
- [mpm] Add `errors="replace"` to all subprocess calls and guard `stdout`/`stderr` against `None`. Windows console programs use the system code page (cp1252 on many runners) rather than UTF-8. Package descriptions that contain cp1252 characters (like `‚` at byte `0x82`) trigger a `UnicodeDecodeError` in Python's subprocess reader thread, which on free-threaded Python 3.14 leaves `result.stdout` as `None` and crashes `_parse_json_array`. `errors="replace"` substitutes invalid bytes with the Unicode replacement character so the reader thread never fails; the `or ""` guards are a defensive belt-and-suspenders measure.
...
- [pwsh-gallery] Add PowerShell Gallery package manager with `installed`, `outdated`, `search`, `install`, `upgrade`, and `remove` support. The implementation drives the modern `Microsoft.PowerShell.PSResourceGet` module (bundled with PowerShell 7.4+) through `pwsh -NoProfile -NonInteractive -Command`, parses JSON via `ConvertTo-Json -AsArray`, and targets `-Scope CurrentUser` for installs so no elevation is required. Cross-platform on Linux, macOS, and Windows. Closes {issue}`1760`.
