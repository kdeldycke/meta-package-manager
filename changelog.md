...
- [mpm] Read-only operations (`installed`, `outdated`, `search`) now query managers in parallel, as does the manager-availability probe, via a new `--jobs`/`-j` option (default: CPU count minus one). The maintenance operations (`sync`, `cleanup`, `upgrade --all`) and the inventory exporters (`dump`/`backup`, `sbom`) parallelize too; the ordering-bound state changers (`install`, `remove`, `upgrade <packages>`, `restore`) stay sequential by design (they dispatch by manager priority) and log an explicit `--jobs` as having no effect. Refs {issue}`529`.
