...
- [mpm] Replace `DETACHED_PROCESS` with `CREATE_NEW_CONSOLE` + `SW_HIDE` when spawning subprocesses on Windows. `DETACHED_PROCESS` broke Chocolatey shims (like `cargo.EXE`) that detect no console and redirect output away from pipes, making version detection fail with "could not parse version". `CREATE_NEW_CONSOLE` gives each child its own isolated console so `GenerateConsoleCtrlEvent(CTRL_C_EVENT, 0)` broadcasts from package-manager installers cannot reach the parent process (uv, pytest), while the child still has a real console for normal output.
- [mpm] Add `errors="replace"` to all subprocess calls and guard `stdout`/`stderr` against `None`. `CREATE_NEW_CONSOLE` causes Windows console programs (like `pwsh`) to use the system code page (cp1252 on many runners) for output rather than UTF-8. Package descriptions that contain cp1252 characters (like `‚` at byte `0x82`) trigger a `UnicodeDecodeError` in Python's subprocess reader thread, which on free-threaded Python 3.14 leaves `result.stdout` as `None` and crashes `_parse_json_array`. `errors="replace"` substitutes invalid bytes with the Unicode replacement character so the reader thread never fails; the `or ""` guards are a defensive belt-and-suspenders measure.
