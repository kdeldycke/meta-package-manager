...
- [mpm] Add an `--orphans` option to `cleanup` that limits the run to each manager's system-wide orphan sweep (`apt autoremove`, `dnf autoremove`, `brew autoremove`, `pkg autoremove`, `xbps-remove --remove-orphans`, `flatpak uninstall --unused`), leaving caches and temporary artifacts untouched. Managers with no such sweep are skipped.
