...
## [`8.0.0` (2026-09-12)](https://github.com/kdeldycke/meta-package-manager/compare/v7.6.1...v8.0.0)
...
- [bar-plugin,gnome-shell] Publish a `Desktop menus` page above the two frontends, carrying what they share: what a click runs, where their settings stop and `mpm`'s configuration starts, the version-diff colors and the bootstrap pair. Both keep their own URL.
...
- [gnome-shell] Add a GNOME Shell extension (GNOME 46 to 50) mirroring the SwiftBar/Xbar plugin: a top bar indicator lists outdated packages per manager, and every menu action runs `mpm` itself. Addresses [#809](https://github.com/kdeldycke/meta-package-manager/issues/809).
...
- [mpm] Bump the in-repo Alpine, MacPorts and Nix packaging specs to build against click-extra `8.6.2` and extra-platforms `13.5.1`, and correct the Alpine package license metadata from `GPL-2.0-or-later` to `Apache-2.0`.
- [mpm] The GitHub issue and PR content-labeller no longer derives `📦 manager:` labels from bare manager IDs, so a pasted `mpm` trace no longer tags an issue with every installed manager; only curated ecosystem keywords a human types now match.
- [cpan] Document why `cpan` reports Perl's core modules as outdated while `upgrade --all` never upgrades them: their versions are tied to the running Perl, cleared only by a writable Perl earlier on `PATH`. Addresses [#1983](https://github.com/kdeldycke/meta-package-manager/issues/1983).
- [gem] Document that on recent macOS the system Ruby's bundled gems surface as outdated yet cannot be upgraded in place, and point to running mpm against a writable Ruby earlier on `PATH` as the fix.
...
- [mpm] Meta Package Manager is now [available in MacPorts](https://ports.macports.org/port/meta-package-manager/) upstream, installable with `sudo port install meta-package-manager` and tested by a new `macports` job.
...
- [mpm] A mutating call of an internal escalator that goes silent for 30 seconds on a terminal, with no warm credential cache, now logs a warning that it may be waiting on a hidden password prompt, quoting its last output line. Addresses [#1938](https://github.com/kdeldycke/meta-package-manager/issues/1938).
...
- [mpm] Streamed CLI output and manager-scoped log lines glue the manager ID into their level prefix (`debug:mas: ...`, `warning:gem: Could not list installed packages.`), making logs grep-able by manager. Closes [#530](https://github.com/kdeldycke/meta-package-manager/issues/530).
...
- [cask] `upgrade --all` now passes `--greedy` to `brew upgrade` when auto-updating packages are included, matching the `outdated` behavior. Auto-updating casks reported as outdated were never upgraded. Closes [#1940](https://github.com/kdeldycke/meta-package-manager/issues/1940).
...
- [mpm] Meta Package Manager is now [available in GNU Guix](https://packages.guix.gnu.org/packages/meta-package-manager/) upstream, installable with `guix install meta-package-manager`.
- [mpm] Rename the reference package to `meta-package-manager`, fetch its source from the tagged git commit, build it with `setuptools.build_meta`, drop the now-optional SBOM and `more-itertools` inputs, and fix the `python-packageurl` input name so `guix install --load-path=packaging/guix` resolves.
- [mpm] Drop the now-optional SBOM and `more-itertools` dependencies from the reference package definition.
...
- [mpm] Add Void Linux build-from-source installation instructions to `docs/install.md`.
...
- [mpm] Move Chocolatey package files into `packaging/choco/meta-package-manager/` so the directory name matches the nuspec basename, fixing the release job.
...
- [mpm] Skip install and remove tests on Linux CI runners: the RPM database at `/var/lib/rpm` is inaccessible on Ubuntu-based runners.
...
- [mpm] Add `mpm` installation instructions with `scoop`.
...
- [mpm] Deactivate integration tests for `mas` on macOS, which always timeout.
...
- [apt,apt-mint] Fix parsing of search results for `apt` and `apt-mint`. Closes [#881](https://github.com/kdeldycke/meta-package-manager/issues/881) and [#966](https://github.com/kdeldycke/meta-package-manager/issues/966).
...
- [zypper] `outdated` returns early when `zypper` prints nothing, instead of handing an empty document to the XML parser.
...
- [flatpak] A package whose version cannot be parsed now reports no version, instead of the literal string `unknown`.
...
- [zypper] Fix `outdated` crashing when `zypper` reports no update at all: the XML traversal assumed the `update-list` element always held a mapping.
...
- [apt,apt-mint] Fix commands incompatible with `--yes` option. Closes [#625](https://github.com/kdeldycke/meta-package-manager/issues/625).
...
- [flatpak] Fix `search` keying its results on the package name instead of the application ID: the two columns were swapped when unpacking a result row.
...
- [mpm] Install `composer` in all platforms CI runners.
...
- [mpm] Fix exceptions when commands gives no output. Closes [#31](https://github.com/kdeldycke/meta-package-manager/pull/31).
- [gem] Fix the exception raised when `gem` returns no output. Closes [#29](https://github.com/kdeldycke/meta-package-manager/issues/29).
...
- [bar-plugin] Move `Upgrade all` menu entry to the bottom of each section in
  BitBar plugin.
- [mpm] Allow destructive unittests in Travis CI jobs.
