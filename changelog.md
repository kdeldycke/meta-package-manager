...
- [mpm] Show a progress spinner on stderr while a manager CLI call runs longer than a second, so a slow operation (like `guix search` scanning every package) no longer looks like a hang. Add a `--progress`/`--no-progress` flag (default on); the spinner auto-suppresses for serialized output, at DEBUG verbosity, without colors, and when stderr is not a terminal. Built on the `Spinner` widget from click-extra.
