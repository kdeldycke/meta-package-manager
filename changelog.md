...
- [mpm] Configuration-defined managers can declare `remove_orphan`, `cleanup_orphan`, `cleanup_cache`, `cleanup_repair` and `orphans` operations, wiring them to `remove --orphans`, the `cleanup` category flags and the `orphans` subcommand.
...
- [flatpak] Plain `cleanup` now also uninstalls unused runtimes and extensions (its orphan category), aligning it with every other orphan-capable manager; previously it only ran `flatpak repair`.
- [xbps] Plain `cleanup` runs its orphan sweep and cache clean as two separate `xbps-remove` invocations instead of one combined command.
