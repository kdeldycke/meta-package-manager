...
## [`8.0.0.dev0` (unreleased)](https://github.com/kdeldycke/meta-package-manager/compare/v7.0.1...main)
...
- [guix] Fix the `packageurl` input name in the reference package (`python-packageurl`, the name it merged under upstream, not `python-packageurl-python`) so `guix install --load-path=packaging/guix` resolves it.
- [nix] Drop the now-optional SBOM and `more-itertools` dependencies from the reference package definition.
- [mpm] Add a global `--network / --no-network` flag (default off) that opts into network calls during a run. `mpm --network sbom` now queries [OSV.dev](https://osv.dev) for known vulnerabilities affecting the inventory and attaches them to the document: CycloneDX gets a `vulnerabilities` array (severity, CVSS vector, CWE ids, aliases, advisory links, with each advisory pointing at every component it affects), SPDX gets per-package `SECURITY` external references. Coverage tracks OSV's indexed ecosystems (pip, npm, cargo, gem, composer); managers OSV does not index get no advisories rather than an error. Responses are cached on disk under the OS user-cache directory, so repeat scans are fast and stay within OSV's rate limits. Network failures, a missing extra, or an unwritable cache degrade gracefully: a warning is logged and the SBOM still renders without vulnerability data. The end-of-run summary gains a `N vulnerabilities found across M packages` line.
- [mpm] Define brand-new package managers from the configuration file. A `[mpm.managers.<id>]` section keyed by an ID that is not a built-in declares a manager from a small TOML schema: its `platforms`, CLI binary and version detection, and an `operations` table mapping each action to a command (with `{package_id}` / `{query}` placeholders) plus, for `installed` / `outdated` / `search`, a per-line `regex` or a JSON parser. Defined managers get their own `--<id>` / `--no-<id>` selectors and join the default set on their supported platforms, like built-ins. Definitions load only from a trusted local config file (owned by you, not world-writable, never a remote `--config` URL); a command-redirecting override (`pre_cmds`, `cli_names`, `cli_search_path`) read from an untrusted source now warns. See the new {doc}`security` page.
- **Breaking:** [mpm] Split the `[sbom]` extra into `[sbom-offline]` (the CycloneDX and SPDX writer libraries, for offline document rendering) and `[sbom-online]` (`httpx` and `platformdirs`, for the `--network` vulnerability lookups). `pip install meta-package-manager[sbom]` no longer resolves; use `[sbom-offline]` for the previous behavior, or `[sbom-offline,sbom-online]` for vulnerability scanning. The standalone binary distribution bundles both via `[tool.repomatic] nuitka.extras`.
- [mpm] The CLI gains a global `--export-config FORMAT` option that prints the fully-resolved configuration (every option's effective value, after the config file, environment, and defaults are merged) as `toml`, `yaml`, `json`, `json5`, `jsonc`, `hjson`, or `xml`, then exits. Redirect it into a file to seed a config from your live setup.
- [mpm] A single Ctrl+C now aborts a concurrent command (`upgrade`, `install`, `outdated`, ...) promptly. Previously, subprocesses that outlived the terminal signal (a `sudo` password prompt, a manager mid-transaction) kept their worker threads alive, so the run hung until a second Ctrl+C and then printed a Python threading-shutdown traceback. The first interrupt now terminates any in-flight package-manager subprocesses, so the thread pool drains and the run exits cleanly.
- [mpm] Privilege escalation is now per-manager and interactive-safe. System package managers (`apt`, `dnf`, `pacman`, `zypper`, `xbps`, ...) still run their state-changing operations through `sudo` by default; a new global `--sudo` / `--no-sudo` flag and a per-manager `[mpm.managers.<id>] sudo` config key override that, letting `npm`/`pip` escalate global installs or a system manager run rootless. Before a state-changing command that will escalate, `mpm` now authenticates `sudo` once up front on the terminal and keeps the credential warm, so a password prompt no longer stalls invisibly inside the concurrent fan-out; off a terminal (pipe, CI, menubar plugin) managers needing root fail fast, with a message naming the manager and pointing at `--sudo`, instead of hanging. Escalated calls run through `sudo -n`. Resolves [#1295](https://github.com/kdeldycke/meta-package-manager/issues/1295) and [#33](https://github.com/kdeldycke/meta-package-manager/issues/33).
...
- [mpm] Add a hermetic test that replays each manager's documented `installed`, `outdated`, and `--version` sample output back through its own parser, so a regex drifting from the `shell-session` example beside it now fails CI. Corrected the stale `flatpak`, `pacstall`, and `asdf` samples it flagged. First step toward [#1023](https://github.com/kdeldycke/meta-package-manager/issues/1023).
- [mpm] The docstring-corpus test now also covers `pwsh-session` blocks (bringing the `choco`, `scoop`, `winget`, and `sfsu` samples into the corpus) and checks every documented mutation command (`install`, `remove`, `upgrade`, `sync`, `cleanup`) against the command the manager actually constructs. Corrected the samples the check flagged across a dozen managers (missing `--quiet`, `--no-ansi`, `--color never`, `--silent`, and post-args). Continues [#1023](https://github.com/kdeldycke/meta-package-manager/issues/1023).
