...
## [`7.4.1.dev0` (unreleased)](https://github.com/kdeldycke/meta-package-manager/compare/v7.4.0...main)

> [!WARNING]
> This version is **not released yet** and is under active development.

- [bar-plugin] Recolor the version-diff suffixes per SwiftBar menu appearance: the default adaptive system red and green lose contrast against the translucent material of recent macOS releases. A light menu now renders both suffixes as fixed, darker shades; a dark menu brightens the green (the red keeps the system color, already the most readable red the palette allows). The appearance is read from the `OS_APPEARANCE` environment variable.
...
- [mpm] Raise the extra-platforms floor from `13.1` to `13.5.1`.
- [mpm] Bump the in-repo Alpine, MacPorts and Nix packaging specs to build extra-platforms from `13.5.1`: the Nix spec drops its coverage and xdist test plugins and per-test skips now that the release runs a plain `pytest` in a hermetic sandbox, and the Alpine build runs the test suite the sdist now ships. The Alpine spec's license metadata is corrected from `GPL-2.0-or-later` to the actual `Apache-2.0`.
- [mpm] Raise the click-extra floor from `8.6` to `8.6.1`.
- [mpm] Bump the in-repo Alpine, MacPorts and Nix packaging specs to build click-extra from `8.6.2`: the Nix spec drops its MkDocs and HOME-leak test skips (`8.6.1` self-skips the MkDocs tree without mkdocs-click and pins `$HOME` in the runner fixture), and the Alpine build runs the full test suite the sdist ships, including the process-group teardown tests that `8.6.2` made robust in a reaper-less container.
...
- [cpan] Document on the manager's page why `cpan` reports its core modules (`warnings`, `feature`, `POSIX`, `B`, and the like) as outdated while `upgrade --all` never upgrades them: these modules ship only inside Perl, with no standalone CPAN distribution, so their versions are tied to the running Perl. On the sealed read-only macOS system Perl the list never clears; on a writable Perl (like Homebrew's) it clears once that Perl catches up, with a recurring lag after each yearly Perl release reaches CPAN before Homebrew bottles it, closed by `brew upgrade perl`. The page also covers disabling the manager. Addresses [#1983](https://github.com/kdeldycke/meta-package-manager/issues/1983).
- [gem] Note on the manager's page that on recent macOS the system Ruby's default gem scope lives on the sealed read-only system volume, so its bundled gems surface as outdated yet cannot be upgraded in place, even with `sudo`. The note points to running mpm against a user-controlled Ruby earlier on `PATH` as the fix.
...
- [macports] Meta Package Manager is now [available in MacPorts](https://ports.macports.org/port/meta-package-manager/) upstream, installable with `sudo port install meta-package-manager` and tested by a new `macports` job.
