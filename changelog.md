...
- [mpm] Lower the default verbosity from `INFO` to `WARNING`: a default run now shows only the `✓`/`✗` trail, finishers, warnings and criticals (the trail prints via `echo`, so it is unaffected). Per-operation narration (`Installation priority`, per-manager announcements, and skip notices like `X does not implement Y` — the latter demoted from `WARNING` to `INFO`) is one `--verbosity INFO` away.
- [mpm] `install` now prints a per-attempt `✓`/`✗` trail (one line per package and manager tried, like `✘ foo not found in brew` then `✓ foo installed with cask`) and a persistent `Installed N/M packages` finisher on an interactive terminal, replacing the per-package `Install … with …` info logs; the per-attempt failure details drop to `DEBUG`.
- [mpm] `remove`, `upgrade`, `sync`, `cleanup` and `restore` gained the same interactive `✓`/`✗` trail and finisher as `install`: per-package for `remove`, `upgrade <packages>` and `restore` (`✓ foo removed from brew`), per-manager for `sync`, `cleanup` and `upgrade --all` (`✓ brew` / `✓ Synced N/M managers`). Per-attempt detail drops to `DEBUG`.
