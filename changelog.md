...
- [mpm] Bump the in-repo Alpine, MacPorts and Nix packaging specs to build click-extra from `8.6.1`: the Nix spec drops its MkDocs and HOME-leak test skips (`8.6.1` self-skips the MkDocs tree without mkdocs-click and pins `$HOME` in the runner fixture), and the Alpine build runs the test suite the sdist now ships.
