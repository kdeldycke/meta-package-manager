...
- [mpm] Surface the `--man` and `--accessible` global options now provided by `click-extra`. `mpm --man` prints the command's man page to stdout in roff (pipe through `man --local-file -` to render), and `--accessible` (also enabled by `ACCESSIBLE=1`) strips ANSI colors and renders tables without Unicode box-drawing characters for screen-reader accessibility. Each tagged release now also attaches an `mpm-manpages.tar.gz` asset with static `.1` files for the full command tree; the Sphinx HTML build emits the same pages alongside the docs site and surfaces a live index at the bottom of [CLI parameters](https://kdeldycke.github.io/meta-package-manager/cli-parameters.html#man-pages). Bumps the `click-extra` floor to `>=7.19`.
- [mpm] Add a `--cooldown DURATION` option and `[mpm] cooldown` config key that refuse to install or upgrade any package version published more recently than the given release age, as a mitigation against supply-chain attacks. Accepts a friendly duration (`7 days`, `1 week`, `12h`), an ISO 8601 duration (`P7D`, `PT12H`), or an RFC 3339 absolute timestamp (`2024-05-01T00:00:00Z`); calendar units like months and years are rejected because their length is ambiguous. Enforced through each manager's native release-age mechanism: `uv` and `uvx` (`UV_EXCLUDE_NEWER`), `npm` (`min-release-age`), and `pip` and `pipx` (`PIP_UPLOADED_PRIOR_TO`). Managers without native support are skipped during install and upgrade by default (fail-closed); pass `--allow-no-cooldown` (or set `[mpm] allow_no_cooldown`) to run them without the safeguard. See the new [Release-age cooldown](https://kdeldycke.github.io/meta-package-manager/cooldown.html) docs page for the per-manager support table. Bumps the minimum required `npm` version from `4.0.0` to `11.10.0` and `pip` version from `10.0.0` to `26.1.0`, the first releases to ship their respective native cooldown gates.
- [brew] Adapt to Homebrew `6.0.0`: pass `--yes` to `brew upgrade` to suppress the new interactive confirmation prompt that hangs automation harnesses like the SwiftBar plugin, and route every tap-qualified install through a new `trust_tap()` helper that runs `brew tap user/tap` (idempotent) then `brew trust user/tap/name --formula` or `--cask` before the install, to avoid `tap trust is required` aborts for third-party packages like `gromgit/fuse/ntfs-3g-mac`. Core formulae and casks on `homebrew/core` and `homebrew/cask` skip the trust step. Bumps the minimum required `brew` version from `2.7.0` to `6.0.0`.
