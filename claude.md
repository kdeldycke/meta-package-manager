...
**`docs/benchmark.toml`** has five alphabetically-sorted keys: `managers` (which competitor supports each manager), `homepages` (URLs for non-pool managers only, none for an archived tool whose site is gone, since a Wikipedia article never stands in for a home page), `coarse_support` (`{manager: {competitor: url}}`), `refused` (`{manager: {competitor: url}}` for competitors that explicitly declined a manager `mpm` wraps), and `unsupported` (`{manager: status}` for managers `mpm` itself declined). `test_benchmark_toml_well_formed` enforces the shape plus the no-orphan and no-conflict invariants (a `(manager, competitor)` pair cannot be in both `managers` and `refused`; an `unsupported` manager must have a `managers` row and must not be in the pool). TOML over YAML is a deliberate choice, not just a rename: `tomllib` rejects a duplicate manager-id row as a parse error, where `yaml.safe_load` used to silently keep the last one.
