...
- 🚧 — **assessed as a candidate, not written yet.** The manager clears the vetting ladder and would be wrapped today if the assessing host had what it needs: an Arch box, a Hyprland session, a Windows machine, or the later date a dead-upstream recheck waits on. Its blocker is written down per manager in the `queued` table of `docs/benchmark.toml`, and landing the wrap is what removes the entry. This is the state a blank cell used to swallow, which left a reader unable to tell a tool nobody had examined from one examined closely and queued.
