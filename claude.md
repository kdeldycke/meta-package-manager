...
- Two shapes: **package-keyed** (`✓ foo installed with brew`, for `install`/`remove`/`upgrade <packages>`/`restore`) and **manager-keyed** (`✓ brew.sync`, `✓ Synced N/M managers`, for `sync`/`cleanup`/`upgrade --all`). `cleanup` suffixes each manager line with the categories dispatched to it (`✓ brew.cleanup (cache)`), since the per-manager subsets differ.
