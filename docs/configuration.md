...
| `sudo`                     | boolean | per-manager         | Force privileged manager operations with (`true`) or without (`false`) `sudo`. Unset by default: system managers (`apt`, `dnf`, `pacman`, ...) escalate, user-level managers do not. On a terminal `mpm` authenticates once up front rather than prompting mid-run; off a terminal, managers needing root fail fast. Per-manager `[mpm.managers.<id>] sudo` overrides this. See [privilege escalation](sudo.md).                                                                                                                                                   |
