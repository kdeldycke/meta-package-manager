...
1. Duplicate its definition file from the [`/managers` subfolder](https://github.com/kdeldycke/meta-package-manager/tree/main/meta_package_manager/managers).
...
1. Fix the code until the unittests and type checking passes. Most metadata, format constraints and structure for new managers are enforced in the unittest suite. See the {doc}`/development` page for more technical details.
1. Submit a PR.
