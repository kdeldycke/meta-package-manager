# {octicon}`package` Chromebrew

```{python:render}
from docs_update import manager_intro

print(manager_intro("chromebrew"))
```

## What `mpm` adds to `chromebrew`

```{python:render}
from docs_update import manager_augments

print(manager_augments("chromebrew"))
```

## Your `chromebrew` commands, in `mpm`

```{python:render}
from docs_update import manager_rosetta

print(manager_rosetta("chromebrew"))
```

## Platforms

```{python:render}
from docs_update import manager_platforms

print(manager_platforms("chromebrew"))
```

## Operations

```{python:render}
from docs_update import manager_operations

print(manager_operations("chromebrew"))
```

## Ecosystem

```{python:render}
from docs_update import manager_ecosystem

print(manager_ecosystem("chromebrew"))
```

## Selecting and configuring `chromebrew`

```{python:render}
from docs_update import manager_selection

print(manager_selection("chromebrew"))
```

## Recipes

```{python:render}
from docs_update import manager_recipes

print(manager_recipes("chromebrew"))
```

## How `mpm` drives `chromebrew`

```{python:render}
from docs_update import manager_cli

print(manager_cli("chromebrew"))
```

## Privilege escalation

```{python:render}
from docs_update import manager_sudo

print(manager_sudo("chromebrew"))
```

## Cooldown

```{python:render}
from docs_update import manager_cooldown

print(manager_cooldown("chromebrew"))
```

## Reference traces

```{python:render}
from docs_update import manager_traces

print(manager_traces("chromebrew"))
```
