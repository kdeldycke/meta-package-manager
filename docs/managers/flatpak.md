# {octicon}`package` Flatpak

```{python:render}
from meta_package_manager._docs import manager_intro

print(manager_intro("flatpak"))
```

## What `mpm` adds to `flatpak`

```{python:render}
from meta_package_manager._docs import manager_augments

print(manager_augments("flatpak"))
```

## Your `flatpak` commands, in `mpm`

```{python:render}
from meta_package_manager._docs import manager_rosetta

print(manager_rosetta("flatpak"))
```

## Platforms

```{python:render}
from meta_package_manager._docs import manager_platforms

print(manager_platforms("flatpak"))
```

## Operations

```{python:render}
from meta_package_manager._docs import manager_operations

print(manager_operations("flatpak"))
```

## Ecosystem

```{python:render}
from meta_package_manager._docs import manager_ecosystem

print(manager_ecosystem("flatpak"))
```

## Selecting and configuring `flatpak`

```{python:render}
from meta_package_manager._docs import manager_selection

print(manager_selection("flatpak"))
```

## Recipes

```{python:render}
from meta_package_manager._docs import manager_recipes

print(manager_recipes("flatpak"))
```

## How `mpm` drives `flatpak`

```{python:render}
from meta_package_manager._docs import manager_cli

print(manager_cli("flatpak"))
```

## Privilege escalation

```{python:render}
from meta_package_manager._docs import manager_sudo

print(manager_sudo("flatpak"))
```

## Cooldown

```{python:render}
from meta_package_manager._docs import manager_cooldown

print(manager_cooldown("flatpak"))
```

## Reference traces

```{python:render}
from meta_package_manager._docs import manager_traces

print(manager_traces("flatpak"))
```
