# {octicon}`package` Haxe haxelib

```{python:render}
from meta_package_manager._docs import manager_intro

print(manager_intro("haxelib"))
```

## What `mpm` adds to `haxelib`

```{python:render}
from meta_package_manager._docs import manager_augments

print(manager_augments("haxelib"))
```

## Your `haxelib` commands, in `mpm`

```{python:render}
from meta_package_manager._docs import manager_rosetta

print(manager_rosetta("haxelib"))
```

## Operations

```{python:render}
from meta_package_manager._docs import manager_operations

print(manager_operations("haxelib"))
```

## Selecting and configuring `haxelib`

```{python:render}
from meta_package_manager._docs import manager_selection

print(manager_selection("haxelib"))
```

## Recipes

```{python:render}
from meta_package_manager._docs import manager_recipes

print(manager_recipes("haxelib"))
```

## Privilege escalation

```{python:render}
from meta_package_manager._docs import manager_sudo

print(manager_sudo("haxelib"))
```

## Cooldown

```{python:render}
from meta_package_manager._docs import manager_cooldown

print(manager_cooldown("haxelib"))
```

## Version probe

```{python:render}
from meta_package_manager._docs import manager_version_probe

print(manager_version_probe("haxelib"))
```

## Reference traces

```{python:render}
from meta_package_manager._docs import manager_traces

print(manager_traces("haxelib"))
```

## Changelog

```{python:render}
from meta_package_manager._docs import manager_changelog

print(manager_changelog("haxelib"))
```
