# {octicon}`package` PowerShell Gallery

```{python:render}
from meta_package_manager._docs import manager_intro

print(manager_intro("pwsh-gallery"))
```

## What `mpm` adds to `pwsh-gallery`

```{python:render}
from meta_package_manager._docs import manager_augments

print(manager_augments("pwsh-gallery"))
```

## Your `pwsh-gallery` commands, in `mpm`

```{python:render}
from meta_package_manager._docs import manager_rosetta

print(manager_rosetta("pwsh-gallery"))
```

## Operations

```{python:render}
from meta_package_manager._docs import manager_operations

print(manager_operations("pwsh-gallery"))
```

## Selecting and configuring `pwsh-gallery`

```{python:render}
from meta_package_manager._docs import manager_selection

print(manager_selection("pwsh-gallery"))
```

## Recipes

```{python:render}
from meta_package_manager._docs import manager_recipes

print(manager_recipes("pwsh-gallery"))
```

## Privilege escalation

```{python:render}
from meta_package_manager._docs import manager_sudo

print(manager_sudo("pwsh-gallery"))
```

## Cooldown

```{python:render}
from meta_package_manager._docs import manager_cooldown

print(manager_cooldown("pwsh-gallery"))
```

## Version probe

```{python:render}
from meta_package_manager._docs import manager_version_probe

print(manager_version_probe("pwsh-gallery"))
```

## Reference traces

```{python:render}
from meta_package_manager._docs import manager_traces

print(manager_traces("pwsh-gallery"))
```

## Upstream project

```{python:render}
from meta_package_manager._docs import manager_upstream

print(manager_upstream("pwsh-gallery"))
```

## Changelog

```{python:render}
from meta_package_manager._docs import manager_changelog

print(manager_changelog("pwsh-gallery"))
```
