# {octicon}`package` Neovim vim-pack

```{python:render}
from meta_package_manager._docs import manager_intro

print(manager_intro("vim-pack"))
```

## What `mpm` adds to `vim-pack`

```{python:render}
from meta_package_manager._docs import manager_augments

print(manager_augments("vim-pack"))
```

## Your `vim-pack` commands, in `mpm`

```{python:render}
from meta_package_manager._docs import manager_rosetta

print(manager_rosetta("vim-pack"))
```

## Platforms

```{python:render}
from meta_package_manager._docs import manager_platforms

print(manager_platforms("vim-pack"))
```

## Operations

```{python:render}
from meta_package_manager._docs import manager_operations

print(manager_operations("vim-pack"))
```

## Ecosystem

```{python:render}
from meta_package_manager._docs import manager_ecosystem

print(manager_ecosystem("vim-pack"))
```

## Selecting and configuring `vim-pack`

```{python:render}
from meta_package_manager._docs import manager_selection

print(manager_selection("vim-pack"))
```

## Recipes

```{python:render}
from meta_package_manager._docs import manager_recipes

print(manager_recipes("vim-pack"))
```

## How `mpm` drives `vim-pack`

```{python:render}
from meta_package_manager._docs import manager_cli

print(manager_cli("vim-pack"))
```

## Privilege escalation

```{python:render}
from meta_package_manager._docs import manager_sudo

print(manager_sudo("vim-pack"))
```

## Cooldown

```{python:render}
from meta_package_manager._docs import manager_cooldown

print(manager_cooldown("vim-pack"))
```

## Reference traces

```{python:render}
from meta_package_manager._docs import manager_traces

print(manager_traces("vim-pack"))
```
