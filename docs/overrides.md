...
| `cooldown_policy`     | string           | Per-manager cooldown posture: `enforce`, `best-effort` or `off`, overriding the globally resolved policy. See [cooldown](cooldown.md).                                               |
...
Every [overridable field](#overridable-fields) (`cli_names`, `cli_search_path`, `requirement`, `version_regexes`, `pre_args`, `extra_env`, `timeout`, ...) may also be set, plus `name`, `homepage_url`, `documentation_url` (the entry point of the tool's own documentation: its manual, its docs site, or the man page of the CLI `mpm` drives), `repository_url` (the repository holding the tool's code), `wikipedia_url` (the English Wikipedia article about the tool) and `logo` (the slug of a brand mark vendored under `docs/assets/managers/`, inlined at the top of that page). When `cli_names` is omitted it defaults to the manager ID.

Ten definition-only fields have no override counterpart:

| Key                     | Type            | Description                                                                                                                                                                                                                                                            |
| :---------------------- | :-------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `brewfile_entry_type`   | string          | Name of the [Homebrew Bundle DSL](https://docs.brew.sh/Brew-Bundle-and-Brewfile) entry this manager maps to, so its installed packages join a [`mpm dump --brewfile`](dump.md#brewfile) export. Omit it for managers with no Brewfile equivalent.                      |
...
| `operation_notes`      | table           | One-sentence hints filling the *Notes* column of the manager page's [Operations table](managers.md), keyed by operation name: why an operation is missing, or what makes a declared one partial.                                                                        |
