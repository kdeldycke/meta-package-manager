...
```{caution}
Building the sdist needs [`uv-build`](https://pypi.org/project/uv-build/), which ships no wheel for every platform and cannot compile where uv itself has no build. FreeBSD on `aarch64` is one such gap: `uv-build` fails there, so `pip install .` from a source tree cannot work, and the published wheel is the only route. Package from the wheel on those platforms, or carry the dependencies and run from the tree.
```

```{note}
FreeBSD splits `sqlite3` out of its base `python3XX` package, and `mpm` imports it through click-extra, so a port must depend on `pyXXX-sqlite3` explicitly or the CLI dies at import with `ModuleNotFoundError: No module named '_sqlite3'`.
```

```{todo}
Drop the `pyXXX-sqlite3` dependency once `mpm` requires a click-extra carrying [kdeldycke/click-extra@c75292d](https://github.com/kdeldycke/click-extra/commit/c75292de7e68184927497c59d513aefd6836ad11), due in `9.1.0`. That commit probes for the `_sqlite3` extension at import time, and moves the `sqlite3` import into the reader that needs it. An interpreter without the bindings then disables the `SQLITE` configuration format alone, and every other format keeps working, so `mpm` installs and starts on a stock FreeBSD Python with no preparation step. Other platforms gain the same guarantee: a distribution can package any standard library module wrapping a system library apart from its interpreter, and click-extra now covers that whole class rather than `sqlite3` alone. Delete the note above and the FreeBSD note of `docs/install.md` in the same change.
```

### click-extra compatibility

`mpm` builds its CLI on [click-extra](https://github.com/kdeldycke/click-extra), and the two evolve in lockstep: most `mpm` releases raise their click-extra floor to pick up features and fixes. The table below shows which click-extra versions each `mpm` release range accepts at install time, derived from the click-extra requirement specifier across release tags. Columns are minor-grouped where the specifiers only distinguish minors, and split per patch where a floor pins a specific patch; both axes read newest-first, so the upper-left corner pairs the latest `mpm` release with the click-extra version currently resolved in `uv.lock`. Rows start at `6.0.0` to keep the column set readable, and the table is regenerated by the same [`matrix` mechanism](https://kdeldycke.github.io/click-extra/python-directives.html#the-matrix-directive) as the [Python compatibility one](install.md#python-compatibility):

<!-- matrix click-extra package=mpm version-floor=6.0.0 show-spec -->

| `mpm`             | Released   | Spec       | `9.2` | `9.0` | `8.8.1` | `8.8.0` | `8.7` | `8.6.1` | `8.6.0` | `8.4` | `8.3` | `8.2` | `8.1.1` | `8.1.0` | `7.19` | `7.16.1` | `7.16.0` | `7.15` | `7.11` | `7.7` | `7.6.2` | `7.6.0` | `7.5.1` | `7.5.0` | `7.2` |
| :---------------- | :--------- | :--------- | :---: | :---: | :-----: | :-----: | :---: | :-----: | :-----: | :---: | :---: | :---: | :-----: | :-----: | :----: | :------: | :------: | :----: | :----: | :---: | :-----: | :-----: | :-----: | :-----: | :---: |
| `8.x`             | 2026-09-20 | `>=9`      |  ✅   |  ✅   |   ❌    |   ❌    |  ❌   |   ❌    |   ❌    |  ❌   |  ❌   |  ❌   |   ❌    |   ❌    |   ❌   |    ❌    |    ❌    |   ❌   |   ❌   |  ❌   |   ❌    |   ❌    |   ❌    |   ❌    |  ❌   |
| `7.6.x`           | 2026-08-10 | `>=8.8.1`  |  ✅   |  ✅   |   ✅    |   ❌    |  ❌   |   ❌    |   ❌    |  ❌   |  ❌   |  ❌   |   ❌    |   ❌    |   ❌   |    ❌    |    ❌    |   ❌   |   ❌   |  ❌   |   ❌    |   ❌    |   ❌    |   ❌    |  ❌   |
| `7.5.0`           | 2026-08-03 | `>=8.7`    |  ✅   |  ✅   |   ✅    |   ✅    |  ✅   |   ❌    |   ❌    |  ❌   |  ❌   |  ❌   |   ❌    |   ❌    |   ❌   |    ❌    |    ❌    |   ❌   |   ❌   |  ❌   |   ❌    |   ❌    |   ❌    |   ❌    |  ❌   |
| `7.4.1`           | 2026-07-28 | `>=8.6.1`  |  ✅   |  ✅   |   ✅    |   ✅    |  ✅   |   ✅    |   ❌    |  ❌   |  ❌   |  ❌   |   ❌    |   ❌    |   ❌   |    ❌    |    ❌    |   ❌   |   ❌   |  ❌   |   ❌    |   ❌    |   ❌    |   ❌    |  ❌   |
| `7.4.0`           | 2026-07-25 | `>=8.6`    |  ✅   |  ✅   |   ✅    |   ✅    |  ✅   |   ✅    |   ✅    |  ❌   |  ❌   |  ❌   |   ❌    |   ❌    |   ❌   |    ❌    |    ❌    |   ❌   |   ❌   |  ❌   |   ❌    |   ❌    |   ❌    |   ❌    |  ❌   |
...
| :--------------- | :--------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------- | :----------- |
| Alpine Linux     | [`packaging/alpine/`](https://github.com/kdeldycke/meta-package-manager/tree/main/packaging/alpine)                    | [pending review](https://gitlab.alpinelinux.org/alpine/aports/-/merge_requests/105566)                    | `apk-source`                                                                         | manual       |
| Arch Linux (AUR) | [community-maintained](https://aur.archlinux.org/packages/meta-package-manager)                                        | released                                                                                                  | [`pacaur`](managers/pacaur.md), [`paru`](managers/paru.md), [`yay`](managers/yay.md) | n/a          |
