...
def no_exact_search(func):
    """Decorator to be used on ``search()`` operations not supporting exact mode."""

    def decorator(self, query, extended, exact):
        if exact:
            logger.warning(f"{self.id} does not implement exact search operation.")
        return func(self, query, extended, exact)

    return decorator


def no_extended_search(func):
    """Decorator to be used on ``search()`` operations not supporting extended mode."""
...
def only_exact_search(func):
    """Decorator to be used on ``search()`` operations only supporting exact mode."""

    def decorator(self, query, extended, exact):
        if not exact:
            logger.warning(f"{self.id} only implements exact search operation.")
        return func(self, query, extended, exact)

    return decorator


def only_extended_search(func):
    """Decorator to be used on ``search()`` operations only supporting extended mode."""

    def decorator(self, query, extended, exact):
        if not exact:
            logger.warning(
                f"{self.id} only implements extended search operation. "
                "Results will be refiltered by mpm."
            )
        return func(self, query, extended, exact)
