...
def _to_int(value: Any) -> int:
    ...
    Rejects `bool` because {class}`bool` is a subclass of {class}`int`
...
Union of every {class}`extra_platforms.Platform` ID and every group ID, so both a
...
- `brewfile_entry_type` maps the manager onto a Homebrew Bundle DSL entry so its
  ...
  {attr}`~meta_package_manager.manager.PackageManager.brewfile_entry_type`).
- `brewfile_skip_warning` is the message emitted when the manager's packages are
  deliberately left out of such an export (see
  {attr}`~meta_package_manager.manager.PackageManager.brewfile_skip_warning`).
- `default_sudo` is the manager's built-in escalation policy (see
  {attr}`~meta_package_manager.execution.CLIExecutor.default_sudo`). Operations
...
(see {meth}`meta_package_manager.manager.PackageManager.cleanup`). A
...
{meth}`meta_package_manager.manager.PackageManager.refiltered_search` narrow the
...
{meth}`meta_package_manager.manager.PackageManager.refiltered_search` still
...
class OperationSpec:
    ...
    managers pass to {meth}`~meta_package_manager.execution.CLIExecutor.run_cli`.
    ...
    `latest_version` (optional). Compiled with {data}`re.MULTILINE`.
    """
...
class ManagerDefinition:
    ...
    {class}`extra_platforms.Platform` members at build time."""
...
class ConfigDrivenManager(PackageManager):
    ...
    declared, so {func}`meta_package_manager.capabilities.implements` reports an
    ...
    Set by {func}`build_bundled_managers` for the managers mpm ships as package
...
def _op_cli_path(manager: PackageManager, spec: OperationSpec) -> Path | None:
    ...
    Thin adapter over
    {meth}`~meta_package_manager.execution.CLIExecutor.sibling_cli`, which
...
def load_bundled_definitions() -> tuple[tuple[ManagerDefinition, str], ...]:
    ...
    {func}`parse_manager_definition`. Returns `(definition, source)` pairs, where
...
def build_bundled_managers() -> list[PackageManager]:
    ...
    Each {class}`ConfigDrivenManager` subclass records the TOML file it came from
    in {attr}`ConfigDrivenManager.definition_source`, so the documentation
    generator can link to it. Called once by
    {attr}`meta_package_manager.pool.ManagerPool.register`.
    """
