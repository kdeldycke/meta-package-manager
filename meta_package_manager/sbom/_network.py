...
{class}`NetworkClient` wraps `httpx` with a filesystem cache and a
bounded retry/backoff policy, so the higher-level adapters (currently
just {mod}`meta_package_manager.sbom.vulnerabilities`, which queries
...
{data}`network_support` reports `False` until the user installs the
`[sbom-online]` extra.
...
class NetworkClient:
    ...
    check {data}`network_support` before constructing, mirroring
    ...
    def client(self) -> httpx.Client:
        ...
        `socksio` installed) is converted to {class}`NetworkError` so
    ...
    def _fetch_with_retries(
        ...
        {class}`NetworkError` once retries are exhausted.
        """
