...
def test_cli_format_plain(invoke, subcommand):
    result = invoke(
        '--output-format', 'json', subcommand, '--cli-format', 'plain')
    for outdated in json.loads(result.output).values():
        for infos in outdated['packages']:
            assert isinstance(infos['upgrade_cli'], str)


def test_cli_format_fragments(invoke, subcommand):
    result = invoke(
        '--output-format', 'json', subcommand, '--cli-format', 'fragments')
    for outdated in json.loads(result.output).values():
        for infos in outdated['packages']:
...
def test_cli_format_bitbar(invoke, subcommand):
    result = invoke(
        '--output-format', 'json', subcommand, '--cli-format', 'bitbar')
    for outdated in json.loads(result.output).values():
        for infos in outdated['packages']:
            assert isinstance(infos['upgrade_cli'], str)
...
@destructive
@unless_macos
def test_unicode_name(invoke, subcommand):
    """ See #16. """
    ...
    result = invoke('--manager', 'cask', subcommand)
    assert result.exit_code == 0
...
@destructive
@unless_macos
def test_multiple_names(invoke, subcommand):
    """ See #26. """
