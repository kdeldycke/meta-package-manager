...
/* The split contract lives in tests/version-diff-cases.json, read here and by
 * test_version.py::test_diff_versions. One list, two implementations: a case
 * added to the file is asserted against both on the next run, which is what
 * keeps this function and the Python diff_versions() from drifting apart. They
 * had, on 17 of 21 pairs, this one leaving the separator introducing the
 * diverging token in the dimmed prefix where the bar plugin colored it. */
const VERSION_DIFF_CASES = JSON.parse(new TextDecoder().decode(
...
function testDiffVersions() {
    ...
    check('diff corpus is populated', VERSION_DIFF_CASES.length > 0, true);
    for (const testCase of VERSION_DIFF_CASES) {
        check(`diff ${testCase.id}`,
            Mpm.diffVersions(testCase.installed, testCase.latest),
            {
                prefix: testCase.prefix,
                oldSuffix: testCase.oldSuffix,
                newSuffix: testCase.newSuffix,
            });
    }
}
