...
def test_manager_count():
    ...
    assert len(manager_classes) == 79
    # Forty-three extra beyond the built-in classes: the bundled config-defined
    # managers (apt-cyg, basalt, bpkg, bun, cargo, cave, choco, chromebrew, clib,
    # cpan, fink, gcloud, getnf, gh-ext, haxelib, jpm, juliaup, krew, macports,
    # micro, ollama, opam, opkg, pamac, pkg-tools, pkgin, pyenv, raco, rustup,
    # slapt-get, soar, sorcery, steamcmd, stew, swupd, tlmgr, topgrade, urpmi,
    # vscode, vscodium, xcodes, yazi, zerobrew), shipped as package data and
    # loaded into the pool at construction.
    assert len(pool) == 122
