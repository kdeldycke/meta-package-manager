...
def test_manager_count():
    ...
    # Thirty-two extra beyond the built-in classes: the bundled config-defined
    # managers (apt-cyg, bin, cargo, cave, choco, chromebrew, cpan, fink,
    # gh-ext, haxelib, juliaup, krew, macports, opam, opkg, pamac, pkg-tools,
    # pkgin, pyenv, rustup, slapt-get, soar, sorcery, steamcmd, stew, swupd,
    # tlmgr, topgrade, urpmi, vscode, vscodium, zerobrew), shipped as package
    # data and loaded into the pool at construction.
    assert len(pool) == 97
