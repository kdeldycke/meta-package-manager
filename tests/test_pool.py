...
def test_manager_count():
    ...
    assert len(manager_classes) == 60
    # Twelve extra beyond the built-in classes: the bundled config-defined managers
    # (cave, chromebrew, fink, gh-ext, pkg-tools, pkgin, slapt-get, soar, sorcery,
    # tlmgr, topgrade, urpmi), shipped as package data and loaded into the pool at
    # construction.
    assert len(pool) == 72
